...
- **Sentences straddling a chunk boundary can fail citation verification even when the model's synthesis is accurate.** `RecursiveChunker` splits on `chunk_size`/`chunk_overlap` (500/150 chars), not sentence boundaries. A sentence longer than `chunk_overlap` that starts near the end of one chunk gets truncated in both that chunk and the next, so its `supporting_quote` can't score above `QUOTE_MATCH_THRESHOLD` against either chunk alone. Diagnosed live: a claim citing "we compare them against human-written code to identify systematic differences [d1]" scored `quote_match_score=0.011` against its chunk because that exact sentence was split mid-word across chunks 60/61 during ingestion. Raising `chunk_overlap` reduces how often this happens but doesn't eliminate it (a sentence longer than the overlap can still split); a real fix would need sentence-aware chunking.
